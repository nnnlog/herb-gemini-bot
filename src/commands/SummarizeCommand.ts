    ...
    public async execute(ctx: CommandContext): Promise<void> {
        ...
        try {
            ...
            if (sentMessages.length > 0) {
                ...
                await logMessage(firstMsg, ctx.botId, CommandType.SUMMARIZE, {parts: result.parts});

                for (let i = 1; i < sentMessages.length; i++) {
                    await logMessage(sentMessages[i], ctx.botId, CommandType.SUMMARIZE, {linkedMessageId: firstMsg.message_id});
                }
            }
