...
async function handleChatCommand(commandMsg: TelegramBot.Message, albumMessages: TelegramBot.Message[] = [], bot: TelegramBot, BOT_ID: number, config: Config, replyToId: number, parsedCommand?: ParsedCommand) {
    const chatId = commandMsg.chat.id;
    try {
        const session = await Session.create(chatId, commandMsg);
        const contentPreparationResult = await prepareContentForModel(bot, commandMsg, albumMessages, 'gemini', session);
