...
describe('chatCommandHandler', () => {
    ...
    it('should handle successful generation', async () => {
        ...
        await handleChatCommand(mockMessage, [], bot, BOT_ID, config, REPLY_TO_ID);
        ...
        expect(mockHandleGeminiResponse).toHaveBeenCalledWith(
            bot, mockMessage, mockResult, BOT_ID, REPLY_TO_ID, 'chat'
        );
    });
    ...
    it('should handle exception', async () => {
        ...
        expect(mockHandleCommandError).toHaveBeenCalledWith(
            error, bot, 999, REPLY_TO_ID, BOT_ID, 'chat'
        );
    });
});
